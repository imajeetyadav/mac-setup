...
# 2. Run the full setup (fresh Mac)
bash quick-mac-setup.sh

# ...or run a single stage (restore one thing on an existing Mac)
bash scripts/03-shell.sh
```

Every stage is idempotent — safe to re-run on an existing machine.
...
| File | Purpose |
|------|---------|
| `quick-mac-setup.sh` | Orchestrator — runs all stage scripts in order |
| `scripts/` | Individual setup stages (run standalone to restore one area) |
